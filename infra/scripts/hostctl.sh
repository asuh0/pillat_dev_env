...
set_php_host() {
    ...
    echo "🔄 Переключение PHP хоста '$host' на $version_input..."
    ...
    if ! runtime_host_compose "$project_dir" build php; then
        echo "Error: не удалось пересобрать образ PHP для '$host'."
        exit 1
    fi

    if [ "$was_running" -eq 1 ]; then
        if ! runtime_host_compose "$project_dir" up -d php; then
            echo "Error: не удалось перезапустить контейнер php для '$host'."
            exit 1
        fi
    ...
    echo "✅ Хост '$host' переключён на PHP $version_input."
    log_event "INFO" "set_php_complete host=$host version=$version_input"
}
