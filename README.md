...
### Переменные окружения проекта (`.env`)

У каждого хоста свой каталог в `projects/<host>/`. Переменные окружения задаются в **файле проекта** `projects/<host>/.env` (при первом создании хоста при необходимости копируется из `.env.example`).

**Корректный процесс, чтобы переменные применялись:**

1. **Где править**  
   Файл `projects/<имя-хоста>/.env` (рядом с `docker-compose.yml` этого хоста). Редактируйте его на хосте (в IDE или вручную).

2. **Формат**  
   Строки вида `ИМЯ=значение`, без кавычек (или в кавычках при пробелах). Комментарии — строки с `#`.

3. **Как они попадают в контейнеры**  
   - Переменные, которые уже есть в `docker-compose.yml` (например `TZ`, `MYSQL_*`, `POSTGRES_*`, `DB_EXTERNAL_PORT`), подхватываются при подстановке в compose и передаются в сервисы БД.
   - Сервис **php** загружает весь файл `.env` через `env_file: .env`, поэтому любые добавленные вами переменные (например `APP_DEBUG=1`, `BITRIX_TYPE=link`) попадают в PHP-контейнер. Для уже созданных проектов при необходимости добавьте в `docker-compose.yml` в секцию сервиса `php` строку `env_file: .env`.

4. **После изменений**  
   Docker Compose читает `.env` при запуске. После правки `.env` перезапустите хост:
   ```bash
   cd infra/scripts
   ./hostctl.sh stop <host>
   ./hostctl.sh start <host>
   ```
   или через DevPanel: Stop → Start для этого хоста.

5. **Частые причины «не применяются»**  
   - Редактировали не тот каталог (должен быть каталог хоста в `projects/`).  
   - Не перезапускали контейнеры после изменения `.env`.  
   - Опечатка в имени переменной в коде (должно совпадать с именем в `.env`).

**Инфраструктура (общая):** глобальные настройки — в `infra/.env.global` (см. быстрый старт). Они используются только для shared-контейнеров (Traefik, Adminer, Grafana и т.д.), не для проектных хостов.
